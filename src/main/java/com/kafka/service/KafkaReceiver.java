...
import com.kafka.bean.MessageDeserializer;
...
public class KafkaReceiver {
    ...
    private Properties getConsumerProperties(){
        ...
        //consumer_group id
        props.put("group.id", "custom-group");
...
//        props.put("auto.commit.interval.ms", "1000");
        ...
        //跟producer端一致
        props.put("value.deserializer", MessageDeserializer.class.getCanonicalName());
    ...
    public KafkaReceiver(String name,boolean isGroupMode){
        ...
        consumer = new KafkaConsumer<String, KafkaMessage>(properties);
    ...
    public void receive(){
        ...
        while (true) {
            try {
                ...
                ConsumerRecords<String, KafkaMessage> records = consumer.poll(100);

                log.info("消费者：{} 获取数据量：" ,this.name, records.count());

                for (ConsumerRecord<String, KafkaMessage> record : records) {
                    log.info("消费者：{} 开始消费数据:offset = {}, key = {}, value = {}", this.name, record.offset(), record.key(), record.value().toString());
                }
                consumer.commitSync();
                //consumer.close();
                Thread.sleep(2000);
